            ...
            if (!user) {
                user = await prisma.user.create({
                    data: {
                        ...
                        password: bcrypt.hashSync(Math.random()
                                                      .toString(36)
                                                      .slice(-8), 10), // Random password
                        githubId: profile.id
                    }
                });
            }
...
export const githubCallback = async (req,
                                     res) => {
    try {
...
passport.use('ldapauth', new LdapStrategy({
    server           : {
        ...
        bindDN         : function (req) {
            ...
            const username = req.body.username.replace(/@vsb\.cz$/, '')
                                .toUpperCase();
...
// VSB login callback
export const vsbCallback = async (req,
                                  res) => {
    try {
        const token = jwt.sign({id: req.user.id}, 'jwtkey');
